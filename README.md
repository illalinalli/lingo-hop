...
- **Начисление XP:** награда за урок копится в `DailyProgress.PendingExperience` и попадает в
  общий счёт **только когда выполнена дневная цель**. Пока цель не закрыта, XP не начисляется —
  сколько бы колод ни было пройдено. Как только цель выполнена, накопленное зачисляется целиком,
  и дальнейшие уроки этого дня начисляются сразу.
- **Уровень:** `1 + XP / 500`, вычисляется, не хранится.
- **Карточка «заучена»** после 3 верных ответов подряд; ошибка сбрасывает серию.
- **Прогресс колоды:** доля карточек, которые человек знает, — `KnownCardCount / CardCount`.
  Карточка считается известной с первого верного ответа и перестаёт считаться при ошибке.
  Знает 1 из 2 карточек — ровно 50%.
